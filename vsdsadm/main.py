...
class Adm:
    def __init__(self):
        self.logger_adm = Log.get_logger('./vsdsadm-v1.0.1/vsdsadm')
    ...
    def create_or_update_linstor_conf(self, controller_ip):
    ...
    def append_fixed_content_to_file(self, password):
    ...
    def create_node(self, node_name, node_ip):
        try:
            result = subprocess.run(["linstor", "node", "create", node_name, node_ip, "--node-type", "Combined"],
    ...
    def create_pv_vg_tp_sp(self, devices, node_name):
        ...
        create_sp = subprocess.run(["linstor", "sp", "c", "lvmthin", node_name, "thpool0", "vg0/tp0"],
    ...
    def adjusting_linstordb(self):
        node_dict = self._count_nodes()
        linstordb_list = self._count_linstordb()
            ...
            if len(nodes) != len(linstordb_list):
                ...
                for i in difference:
                    create_res = subprocess.run(
                        ["linstor", "r", "c", i, "linstordb", "--storage-pool", node_dict[i][0]], capture_output=True,
                        text=True)
                    if create_res.returncode != 0:
    ...
    def adjusting_pvc(self):
        node_dict = self._count_nodes()
        pvc_dict = self._count_pvc()
        if pvc_dict == {}:
            print("没有pvc相关资源")
            return
        nodes = list(node_dict.keys())
        ...
        for pvc_name in pvc_nodes:
            if len(nodes) >= 3:
                ...
                if len(nodes) != len(pvc_dict[pvc_name]):
                    ...
                    for i in difference:
                        create_res = subprocess.run(
                            ["linstor", "r", "c", i, pvc_name, "--storage-pool", node_dict[i][0]],
                            capture_output=True, text=True)
                        if create_res.returncode != 0:
